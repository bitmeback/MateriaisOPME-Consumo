...
(async () => {
  ...
  try {
    console.log('[MIGRACAO] Pedidos Mat. Cirurgias: Jan/2024 até', currentMonth + '/' + currentYear);
    ...
    for (let loopYear = startYear; loopYear <= currentYear; loopYear++) {
        ...
        for (let loopMonth = 1; loopMonth <= mesFinalScope; loopMonth++) {
            ...
            console.log(`[DAX] Carga: MÊS ${loopMonth} | ANO ${loopYear}`);

            // Adicionado colunas extras para englobar as views de pacote,nf,impostos
            const daxQuery = `
              EVALUATE 
              SUMMARIZECOLUMNS(
            ...
            console.log(`  - Extraídos ${rows.length} registros`);
            ...
            if (rows.length > 0) {
                ...
                await connection.execute(`DELETE FROM pedidos_mat_cirurgias WHERE ano = ? AND mes = ?`, [loopYear, loopMonth]);
                
                const chunkSize = 2000;
                for (let i = 0; i < valuesArray.length; i += chunkSize) {
                    await connection.query(
                        `INSERT INTO pedidos_mat_cirurgias (nr_atendimento, nr_cirurgia, cd_material, ds_material, ds_pendencias, qtde, vl_conta, vl_ultima_compra, vl_lucro_liq, vl_taxa_prop, vl_imposto, vl_oc, ds_setor_atendimento, ds_motivo_baixa, ds_carater, ds_convenio, ds_fornecedor, ie_pacote, situacao, dt_termino, dt_baixa, nr_interno_conta, nr_nf_ent_consignado, tipo, ano, mes) VALUES ?`,
    ...
    console.log('[CONCLUÍDO] MIGRAÇÃO FINALIZADA. 🎉');
