...
(async () => {
  ...
  // Usar SUMMARIZECOLUMNS com CD_MATERIAL e como segunda coluna usar防汛 no Saldo 灾 款
  // Adiciono uma medida de contagem para garantir que a query não agregue tudo
  const daxAll = `
    EVALUATE
    SUMMARIZECOLUMNS(
  ...
  console.log("Total de linhas (material x fornecedor): " + rows.length);
  console.log("Amostra das 5 primeiras linhas:");
  rows.slice(0, 5).forEach((r, i) => {
    console.log("  " + JSON.stringify(r));
  });
  console.log("Amostra das 5 últimas linhas:");
  rows.slice(-5).forEach((r, i) => {
    console.log("  " + JSON.stringify(r));
  });

  // Testar: quantos materiais únicos?
  const cds = new Set();
  rows.forEach(r => cds.add(r["SALDO_ESTOQUE[CD_MATERIAL]"]));
  console.log("\nMateriais únicos: " + cds.size);
